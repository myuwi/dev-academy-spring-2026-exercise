...
When doing this assignment, I noticed that the different columns in the database seem to use a mix of different time zones (UTC and Europe/Helsinki?), as when inspecting the different cutoffs for `NULL` values and the `starttime` column, they do not seem to line up.

For example, when assuming that the values in the `date` and `starttime` columns use local time (i.e., that they are already in Europe/Helsinki time zone), the `NULL` values in the `consumptionamount` column first stop and start again at midnight, between `2023-07-31` and `2023-08-01`, and `2024-09-29` and `24-09-30` respectively, which seems reasonable.
However, when inspecting the `starttime` column, the first recorded time is 22:00 instead of 00:00, and the last recorded time is 20:00, instead of 23:00, which makes me think that the `startime` column is in UTC, while `consumptionamount` (as well as `hourlyprice`?) are in Europe/Helsinki.

Originally, I wanted to translate the times from UTC to the Europe/Helsinki time zone using a database view and "fix" the first and last days, but as this makes the consumption amount and average price cutoffs happen in the middle of a day, I did not do this in the final version on the `main` branch (and thus the version running in the cloud). The code for translating the dates from UTC to Europe/Helsinki using a database view is however available on a separate `timezones` branch. If the intention was for us to notice this and translate the timestamps into the correct time zone before aggregating the data, feel free to switch to the `timezones` branch where the time zones are accounted for, and evaluate that version instead :)
